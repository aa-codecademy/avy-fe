                ...
                <div class="flex items-start gap-4">
                    ...
                    <div class="flex-1">
                        <div class="flex justify-between items-start mb-2 flex-wrap gap-2">
                            ...
                            <div>
                                ${renderStatusBadge(app.status, t)}
                            </div>
                        </div>

                        <div class="flex flex-wrap gap-2 mb-3">
                            <span class="px-3 py-1 bg-gray-100 text-gray-700 text-sm rounded-full">
                                <i class="fas fa-calendar-alt mr-1"></i> ${t('applications.applied')}: ${formatDate(app.appliedDate)}
                            </span>
                            ${
                        ...
                        ${
                            app.status === 'accepted'
                                ? `
                            <div class="text-sm text-green-700 mb-3 bg-green-50 p-2 rounded border border-green-200">
                                <i class="fas fa-check-circle mr-1 text-green-600"></i>
                                <strong>${t('applications.offerAccepted')}</strong> ${app.notes ? `- ${escapeHtml(app.notes.startsWith('applications.') ? t(app.notes) : app.notes)}` : ''}
                            </div>
                        `
                                : app.status === 'rejected'
                                  ? `
                            <div class="text-sm text-red-700 mb-3 bg-red-50 p-2 rounded border border-red-200">
                                <i class="fas fa-times-circle mr-1 text-red-600"></i>
                                ${app.notes ? (app.notes.startsWith('applications.') ? t(app.notes) : escapeHtml(app.notes)) : t('applications.positionFilled')}
                            </div>
                        `
                                  : app.notes
                                    ? `
                            <div class="text-sm text-gray-600 mb-3 bg-gray-50 p-2 rounded">
                                ...
                                ${app.notes.startsWith('applications.') ? t(app.notes) : escapeHtml(app.notes)}
                            </div>
                        `
                                    : ''
                        }
                        ...
                        <div class="flex justify-between items-center flex-wrap gap-2">
                            <div class="text-sm text-gray-500">
                                ...
                                ${t('applications.lastUpdate')}: ${formatDate(app.updatedAt || app.appliedDate)}
                            </div>
                            ${
                                canWithdraw
                                    ? `
                                <button class="withdraw-btn px-4 py-2 bg-red-50 text-red-600 hover:bg-red-100 rounded-lg transition text-sm font-medium"
                                    ...
                                    <i class="fas fa-times-circle mr-1"></i> ${t('buttons.withdraw')}
                                </button>
                            `
                                    : app.status === 'declined' || app.status === 'withdrawn'
                                      ? `
                                <span class="text-sm text-gray-400">
                                    <i class="fas fa-check-circle mr-1"></i> ${t('applications.cancelled')}
                                </span>
                            `
                                      : ''
                            }
