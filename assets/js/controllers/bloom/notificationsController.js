...
function filterNotifications(notifications, filter) {
    switch (filter) {
        ...
        case 'applications':
            return notifications.filter((n) => APPLICATION_TYPES.includes(n.type));
        default:
...
function renderNotificationRow(n) {
    ...
    const rowBg = !n.read ? 'bg-white border-l-4 border-indigo-500' : 'bg-gray-50';
    ...
    function attachEvents() {
        ...
        document.querySelectorAll('.pref-enabled').forEach((input) => {
            input.addEventListener('change', (e) => {
                ...
                document
                    .querySelectorAll(`input[name="channel-${k}"]`)
                    .forEach((r) => (r.disabled = !draft[k].enabled));
            });
        });
